...
export function TopBar({ onMenu }: { onMenu: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 px-5 py-3 bg-bg/80 backdrop-blur border-b border-line">
      <button onClick={onMenu} aria-label="메뉴 열기" className="md:hidden w-9 h-9 rounded-xl bg-white border border-line flex items-center justify-center text-ink">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M4 12h16M4 17h16" stroke="#2B2D42" strokeWidth="2" strokeLinecap="round"/></svg>
      </button>
      <button onClick={() => router.push('/search')} aria-label="공지 검색"
      ...
      <div className="ml-auto flex items-center gap-2">
        <Link href="/saved" aria-label="알림" className="relative w-9 h-9 rounded-xl bg-white border border-line flex items-center justify-center">
