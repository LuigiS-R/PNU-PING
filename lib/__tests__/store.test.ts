...
describe('store', () => {
  ...
  it('logout은 상태를 초기화', () => {
    ...
    s.login('a@pusan.ac.kr'); s.subscribe('cse'); s.toggleSave('n1')
    usePingStore.getState().logout()
    const state = usePingStore.getState()
    expect(state.loggedIn).toBe(false)
    expect(state.email).toBeUndefined()
    expect(state.subscribedDeptIds).toEqual([])
    expect(state.savedNoticeIds).toEqual([])
    expect(state.settings).toEqual({ newNotice: true, importantOnly: false, pingVoice: true })
  })
})
