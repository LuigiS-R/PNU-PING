...
- **기능 범위: Notion 보고서의 모든 사용자 기능을 빠짐없이 구현** (가입·학과구독·구독해제·통합피드(전체/구독)·대학→학과 탐색·알림). §5-1 매핑 표 참조.
- **디자인: `ping-design-system-3.html` 스타일 그대로** (컬러/타이포/카드/마스코트/라운드/그림자).
- 데이터: 샘플(mock) JSON + localStorage — 백엔드 없이 사용자 기능이 완전 동작
...
## 5-1. Notion 기능 → 구현 매핑 (전부 구현 · 백엔드는 클라이언트 시뮬레이션)

| Notion 기능 | 구현 위치 | 비고 |
|---|---|---|
| 가입 (이메일 기반) | `/login` → `/onboarding` | @pusan.ac.kr 이메일 입력 + 구글 버튼(목업). 가입 정보(이메일+구독학과)를 store→localStorage 저장 = "구독 관리 서비스" stand-in |
| 학과 구독 / 학과 추가 | Sidebar ＋학과추가 모달, `/onboarding`, `/settings` | 단과대→학과 메뉴 선택, 최대 4개 |
| 구독 해제 | `/settings` 구독 학과 관리 | 해제/추가 |
| 통합 피드 — 모든 학과 | `/feed`(전체), `/` 홈 | mock JSON = "공지 JSON 피드 API" stand-in |
| 통합 피드 — 구독 학과 | `/feed`(나의 학과), Sidebar ⭐나의학과 | 로그인 시 |
| 대학별 버튼 → 학과 나열 | Sidebar 단과대학 ▸ 학과 아코디언 | Notion 화면1 |
| 학과별 버튼 → 피드 갱신 | Sidebar 학과 클릭 → 피드 컨텍스트 전환 | Notion 화면1 |
| 새 공지 알림(이메일 발송) | TopBar 🔔 알림 + `/settings` 알림 토글 | 실제 이메일 발송은 백엔드(후속); 프론트는 in-app 알림으로 표현 |

> 위 사용자 기능은 본 단계에서 **mock 데이터 + localStorage**로 완전 동작하게 구현한다.
> 백엔드 인프라(RSS 폴링·JSON 변환·메시지 큐·이메일 발송·실제 DB/API·실제 OAuth)는 §10대로 후속.
